...
import { ArrowLeft, Sparkles, Send, Copy, Check, RefreshCw, Code, Zap, MessageSquare, Shield, Wand2, FileJson, AlertCircle } from 'lucide-react';

interface JSONToolProps {
  onBack: () => void;
}

export const JSONTool: React.FC<JSONToolProps> = ({ onBack }) => {
      ...
      <header className="h-20 px-8 flex items-center justify-between bg-white/5 border-b border-white/10 shrink-0 z-50">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-black text-white uppercase tracking-widest leading-none">JSON Formatter</h2>
              <p className="text-[9px] font-bold text-slate-500 uppercase tracking-[0.2em] mt-1">Structure Validation Engine</p>
            </div>
          </div>
        </div>
      </header>
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 h-full">
          ...
          <div className="flex flex-col gap-6">
            <div className="flex-1 bg-white/5 rounded-[2.5rem] border border-white/10 p-8 flex flex-col relative overflow-hidden group">
              ...
              <div className="flex items-center justify-between mb-6 relative z-10">
                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Raw JSON Input</h3>
              </div>
              ...
              <button 
                ...
                disabled={!input.trim()}
                className="mt-6 h-14 bg-cyan-500 hover:bg-cyan-400 disabled:bg-slate-800 disabled:text-slate-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] flex items-center justify-center gap-3 shadow-2xl shadow-cyan-500/20 transition-all active:scale-95 relative z-10"
              >
                <Zap size={16} />
                Format & Validate
              </button>
          ...
          <div className="flex flex-col gap-6">
            <div className="flex-1 bg-slate-900 rounded-[2.5rem] border border-white/5 p-8 flex flex-col relative overflow-hidden group">
              ...
              <div className="flex items-center justify-between mb-6 relative z-10">
                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Formatted Output</h3>
                {output && (
                  <button 
              ...
              <div className="flex-1 overflow-y-auto pr-4 scrollbar-hide relative z-10">
                {error ? (
                  <div className="p-6 bg-rose-500/10 border border-rose-500/20 rounded-2xl flex items-start gap-4">
                    ...
                    <div>
                      <h4 className="text-xs font-black text-rose-500 uppercase tracking-widest mb-1">Validation Error</h4>
                      <p className="text-[10px] font-bold text-rose-400/60 uppercase leading-normal">{error}</p>
                    </div>
                  </div>
                ) : output ? (
                  <pre className="text-cyan-400 text-sm font-mono leading-relaxed whitespace-pre-wrap">
                    {output}
                  </pre>
                ) : (
              ...
              <div className="mt-6 p-4 bg-white/5 rounded-2xl border border-white/5 flex items-center gap-3 relative z-10">
                ...
                <p className="text-[9px] font-bold text-slate-500 uppercase tracking-tight">
                  Structural integrity verified. Pretty-print active.
                </p>
              </div>
            </div>
          </div>
